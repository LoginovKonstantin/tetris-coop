...
function generateTetromino() {
  ...
  const x = Math.floor((GRID_WIDTH - shape[0].length) / randomNum);
...
// Reset game state
function resetGame() {
  gameState.grid = Array(GRID_HEIGHT)
    .fill()
    .map(() => Array(GRID_WIDTH).fill(0));
...
function notify() {
  ...
  axios
    .get(
      `${url}/bot${token}/sendMessage?parse_mode=html&chat_id=${chatId}&text=${message}`
    )
...
io.on('connection', (socket) => {
  ...
  socket.emit('init', {
    grid: gameState.grid,
    tetromino: gameState.players[socket.id].tetromino,
